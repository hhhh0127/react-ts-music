 ...
 */
export function addPlaylistId(id: any[] | number, key = LOCAL_PLAYLIST_ID_KEY) {
 ...
 */
export function getPlaylistId(key: string = LOCAL_PLAYLIST_ID_KEY) {
  const songListId = localStorage.getItem(key)
    ? JSON.parse(localStorage.getItem(key) || '')
    : []
  return songListId
 ...
 * @param {String} key
 */
export function removeSongId(id: number | string, key = LOCAL_PLAYLIST_ID_KEY) {
  const songListId = localStorage.getItem(key)
    ? JSON.parse(localStorage.getItem(key) || '')
    : []
  // 数组有值 & 查找到了要移除的id
